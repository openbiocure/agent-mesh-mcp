...
export function formatDeployNudge(d) {
  let text = `📋 Release ${d.shortId}: ${d.status}\n${d.name}`;
  if (d.elapsedMinutes) text += `\nDeploying for ${d.elapsedMinutes}min`;
  if (d.summary) text += `\n\n${d.summary.slice(0, 500)}`;
  if (d.lastActivity) {
    text += `\n\nLast devops activity (${d.lastActivity.agoSeconds}s ago): ${d.lastActivity.content?.slice(0, 200) || "unknown"}`;
  }
  return text;
}

export function formatApprovalComment(data) {
  return `💬 Comment added to ${data.approvalShortId}${data.agentNotified ? " — agent notified" : ""}`;
}

export function formatReplyRouted(data) {
  return `↩️ Reply sent to ${data.workerName}`;
}
