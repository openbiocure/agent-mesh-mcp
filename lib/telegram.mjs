...
export async function telegramWebhookHandler(req, res) {
  ...
  try {
    ...
    if (!callback) {
      ...
      if (message?.text?.startsWith("/")) {
        ...
        if (cmd === "/releases") {
          ...
          if (!shortId) {
            // Show active releases with subscribe buttons
            const releases = await prisma.release.findMany({
              where: { status: { in: ["building", "ready", "deploying"] } },
              orderBy: { createdAt: "desc" },
              take: 10,
            ...
            if (releases.length === 0) {
              reply = "No active releases to subscribe to.";
            } else {
          ...
          const shortId = message.text.split(" ")[1];
          if (shortId === "all") {
            const result = await prisma.$executeRaw`DELETE FROM subscriptions WHERE chat_id = ${String(message.chat.id)}`;
            reply = result > 0 ? `Unsubscribed from all (${result} removed).` : `No active subscriptions.`;
          } else if (shortId) {
            ...
            // No arg — show active subscriptions with buttons
            const subs = await prisma.$queryRaw`
              SELECT s.entity_id as "entityId", r.name FROM subscriptions s
              LEFT JOIN releases r ON r.id::text = s.entity_id
              WHERE s.chat_id = ${String(message.chat.id)}
              ORDER BY s.created_at DESC`;
            if (subs.length === 0) {
              reply = "No active subscriptions.";
            } else {
              const buttons = subs.map(s => [{ text: `❌ ${s.name || s.entityId.slice(0, 8)}`, callback_data: `unsub:${s.entityId}` }]);
              ...
              await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
                ...
                body: JSON.stringify({
                  chat_id: message.chat.id,
                  text: `🔔 Your subscriptions (${subs.length}):`,
                  reply_markup: { inline_keyboard: buttons },
                }),
              });
              res.json({ ok: true });
          ...
          reply = newVal === "true" ? "🔇 Deploy schedule silenced — only notifies on actual deploys" : "🔊 Deploy schedule verbose — notifies every cycle";
        } else if (cmd === "/help") {
          reply = "/releases — active releases\n/agents — online agents\n/incidents — open incidents\n/subscribe — subscribe to release updates\n/unsubscribe — manage subscriptions\n/quiet — toggle deploy schedule notifications\n/help — this message";
        }
      ...
      // Reply to a message — route back to agent, nudge, or comment
      if (message?.reply_to_message) {
        const origText = message.reply_to_message.text || "";
        const origMsgId = message.reply_to_message.message_id;

        // Check if this reply is to a notification — route back to the agent
        if (origMsgId && message.text) {
          const notifs = await prisma.$queryRaw`SELECT worker_name as "workerName", worker_sid as "workerSid", type, title FROM notifications WHERE message_id = ${origMsgId} LIMIT 1`;
          if (notifs.length === 1) {
            ...
            try {
              ...
              await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ chat_id: message.chat.id, text: `↩️ Reply sent to ${notif.workerName}`, reply_to_message_id: message.message_id }),
              });
            } catch (e) {
    ...
    if (action === "unsub") {
      ...
      let result;
      if (callbackId === "all") {
        result = await prisma.$executeRaw`DELETE FROM subscriptions WHERE chat_id = ${chatId}`;
      } else {
        result = await prisma.$executeRaw`DELETE FROM subscriptions WHERE entity_id = ${callbackId} AND chat_id = ${chatId}`;
      }
      await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/answerCallbackQuery`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ callback_query_id: callback.id, text: result > 0 ? "Unsubscribed" : "Already unsubscribed" }),
      });
      await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/editMessageText`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ chat_id: callback.message.chat.id, message_id: callback.message.message_id, text: callbackId === "all" ? "🔕 All subscriptions removed." : "🔕 Unsubscribed." }),
      });
      res.json({ ok: true });
      return;
    }
    ...
    if (action === "sub") {
      const chatId = String(callback.from.id);
      const existing = await prisma.$queryRaw`SELECT id::text FROM subscriptions WHERE entity_type = 'release' AND entity_id = ${callbackId} AND chat_id = ${chatId}`;
      if (existing.length === 0) {
        await prisma.$executeRaw`INSERT INTO subscriptions (entity_type, entity_id, chat_id) VALUES ('release', ${callbackId}, ${chatId})`;
      }
      const rel = await prisma.$queryRaw`SELECT name FROM releases WHERE id::text = ${callbackId}`;
      await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/answerCallbackQuery`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ callback_query_id: callback.id, text: "Subscribed!" }),
      });
      await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/editMessageText`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ chat_id: callback.message.chat.id, message_id: callback.message.message_id, text: `🔔 Subscribed to ${rel[0]?.name || callbackId.slice(0, 8)}` }),
      });
      res.json({ ok: true });
      return;
    }

    const approvalId = callbackId;
