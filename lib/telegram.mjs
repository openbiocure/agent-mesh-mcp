  ...
  try {
    ...
    if (!callback) {
      ...
      if (message?.reply_to_message && message?.text) {
        ...
        if (match) {
          const approvals = await prisma.$queryRaw`SELECT id::text as id FROM approvals WHERE id::text LIKE ${match[1] + "%"}`;
          if (approvals.length === 1) {
            ...
            await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
              ...
              body: JSON.stringify({ chat_id: TELEGRAM_CHAT_ID, text: `\u{1F4AC} Comment added to ${match[1]}` }),
            });
          }
        }
      }
